...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderSourceActions(project);
  setupSourceCodeModal(project);
});
...
/* -------------------------------------------------------------------
   Source actions — "View Source Code" (opens the in-page codespace
   modal below) and "View Repository" (jumps straight to GitHub).
   Either button hides itself if the project doesn't have that data,
   so this degrades gracefully for older/incomplete project records.
------------------------------------------------------------------- */
function renderSourceActions(project) {
  const wrap = document.getElementById('pdSourceActions');
  const viewSourceBtn = document.getElementById('viewSourceBtn');
  const viewRepoBtn = document.getElementById('viewRepoBtn');
  if (!wrap) return;

  const hasFiles = !!(project.sourceCode && project.sourceCode.files && project.sourceCode.files.length);
  const hasRepo = !!project.sourceUrl;

  if (viewSourceBtn) viewSourceBtn.hidden = !hasFiles;
  if (viewRepoBtn) {
    if (hasRepo) {
      viewRepoBtn.href = project.sourceUrl;
      viewRepoBtn.hidden = false;
    } else {
      viewRepoBtn.hidden = true;
    }
  }
  wrap.hidden = !(hasFiles || hasRepo);

  setText('srcModalTitle', `${project.title} — Source Code`);
  const githubBtn = document.getElementById('srcModalGithubBtn');
  if (githubBtn) {
    if (hasRepo) {
      githubBtn.href = project.sourceUrl;
...
/* -------------------------------------------------------------------
   Source code modal — one reusable "codespace" viewer shared by every
   project. Renders a clickable file list on the left (works for a
   single file or dozens) and the selected file's content on the
   right. No per-project HTML page is ever created.
------------------------------------------------------------------- */
function setupSourceCodeModal(project) {
  const fileListEl = document.getElementById('srcFileList');
  const filenameEl = document.getElementById('srcActiveFileName');
  const codeEl = document.getElementById('srcCodeContent');
  const copyBtn = document.getElementById('srcCopyBtn');
  if (!fileListEl || !codeEl) return;

  const files = (project.sourceCode && project.sourceCode.files) || [];
  ...
  function renderFileList() {
    fileListEl.innerHTML = files.map((file, i) => `
      <button class="src-file-btn${i === activeIndex ? ' is-active' : ''}" type="button" data-index="${i}">
        <span class="src-file-btn__ext">${escapeHTML(fileExt(file.name))}</span>
        <span class="src-file-btn__name">${escapeHTML(file.name)}</span>
      </button>
    `).join('');

    fileListEl.querySelectorAll('.src-file-btn').forEach((btn) => {
